#!/usr/bin/env python3
"""Scout one move beyond the radius-3 q=1 obstruction-filter frontier."""
...
def source_priority(record: dict[str, Any]) -> tuple[Any, ...]:
    spectrum = record["spectrum_certificate"]["vectorlike_prediction"]
    return (
        record["classification"]["status"] != "negative_control_doublet_triplet_obstruction",
        not record["spectrum_certificate"]["desired_q1_three_family_signature"],
        spectrum.get("h2_wedge2_regular_multiplicity") is None,
        spectrum.get("h2_wedge2_regular_multiplicity", 99),
        source_key(record),
    )
...
def priority_key(item: dict[str, Any]) -> tuple[Any, ...]:
    return (
        ...
        item["source_desired_q1"] is not True,
...
def build_adjacency_frontier(
    *,
    ...
    counters = {
        "source_radius3_records": len(source_records),
    ...
    for source in source_records:
        ...
        for primitive in primitives:
            ...
            frontier.append(
                {
                    "source_raw_candidate_key": source_key(source),
                    "source_final_label": source["label"],
                    "source_obstruction_status": source["classification"]["status"],
                    "source_desired_q1": source["spectrum_certificate"][
...
def build_report(
    *,
    ...
    radius3_filter = load_json(
        REPORTS / "phenomenology_guided_q1_radius3_obstruction_filter_certificate.json"
    )
    ...
    source_records = [
        record
        for record in radius3_filter["strict_candidate_records"]
        if record["classification"]["category"] == "phenomenologically obstructed"
    ]
    source_records.sort(key=source_priority)
    total_source_records = len(source_records)
    source_records = source_records[source_start:]
    if source_limit > 0:
    ...
    for index, raw in enumerate(raw_q1_records[:max_raw_q1_to_certify]):
        ...
        certified = certify_5259_matrix(
            label=f"radius4_adjacency_certified_{index}",
            matrix=raw["matrix"],
            move=[raw["new_move"]],
            slope_restarts=certification_restarts,
            slope_seed=seed + 900000 + index,
        )
        certified["radius4_source_raw_candidate_key"] = raw["source_raw_candidate_key"]
        certified["radius4_source_obstruction_status"] = raw[
            "source_obstruction_status"
        ]
        certified["radius4_new_move"] = raw["new_move"]
        ...
        filtered = candidate_certificate_from_5259_record(
            label=f"radius4_adjacency_filtered_{index}",
            record=certified,
            conf=conf,
        )
        ...
        filtered["radius4_source_raw_candidate_key"] = raw["source_raw_candidate_key"]
        filtered["radius4_source_obstruction_status"] = raw[
            "source_obstruction_status"
        ]
        filtered["radius4_new_move"] = raw["new_move"]
    ...
    gates = {
        "radius3_filter_imported": gate(
            radius3_filter["all_gates_pass"]
            and radius3_filter["summary"]["effective_viable"] == 0
            and radius3_filter["summary"]["strict_raw_q1_survivors"] == 64,
            str(REPORTS / "phenomenology_guided_q1_radius3_obstruction_filter_certificate.json"),
            "scout expands the verified radius-3 obstruction-filter ledger",
        ),
        "adjacency_frontier_generated": gate(
            adjacency_counters["raw_edges"] == len(source_records) * len(primitives)
            and adjacency_counters["unique_candidates"] > 0
            and len(screened) > 0,
            "radius-4 adjacency frontier",
            "one additional primitive move was generated from the selected radius-3 q=1 records",
        ),
    ...
    return {
        "scope": "bounded radius-4 adjacency scout from the radius-3 q=1 obstruction frontier",
        "status": (
            "viable_candidate_found"
            if viable
            else "no_viable_candidate_found_in_bounded_radius4_adjacency_scout"
        ),
        "search_parameters": {
            "source": "phenomenology_guided_q1_radius3_obstruction_filter_certificate.json",
            "move_radius_interpretation": "one primitive move beyond selected strict radius-3 q=1 records",
            "total_available_source_records": total_source_records,
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        ...
        "",
        (
            "This is a bounded one-move expansion window from selected strict "
            "radius-3 q=1 obstruction records, not a full radius-4 no-go."
        ),
        "",
        "## Counts",
        "",
    ...
    for record in report["filtered_candidate_records"]:
        lines.append(
            "- "
            f"`{record['label']}` from `{record['radius4_source_raw_candidate_key']}`: "
...
def main() -> int:
    ...
    parser.add_argument("--max-frontier-to-screen", type=int, default=3000)
    ...
    parser.add_argument("--seed", type=int, default=525979144)
