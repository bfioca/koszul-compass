#!/usr/bin/env python3
"""Cumulative selected radius-4 frontier after six verified branch closures."""
...
def build_report() -> dict[str, Any]:
    ...
    gates = {
        "all_closure_verifications_pass": gate(
            ...
            "cumulative5 frontier subtracts only verified no-viable branch closures",
        ),
        "cumulative_counts_match": gate(
            len(open_candidates) == 12
            and sum(row["block_count"] for row in open_rows) == 38
            and len(open_rows) == 18,
            "cumulative5 open candidate/orbit rows",
            "fifteen branch-closed candidates and seven Serre orbits are removed",
        ),
        "open_distribution_match": gate(
            dict(sorted(open_distribution.items())) == {1: 9, 2: 2, 6: 1},
            "cumulative5 open candidates",
            "open candidate missing-orbit distribution matches cumulative5 subtraction",
        ),
        "next_open_orbit_match": gate(
            open_rows[0]["serre_orbit_representative"] == [0, 0, -1, -3, 0, 1, 1]
            and open_rows[0]["candidate_count"] == 2
            and open_rows[0]["block_count"] == 4,
            "cumulative5 open Serre orbit ranking",
            "next cumulative5 bottleneck is the remaining two-candidate dimension-six orbit",
        ),
        "closed_items_absent": gate(
            ...
            "cumulative5 open frontier",
            "closed candidates and Serre orbits are absent",
        ),
    }
    return {
        "scope": "cumulative5 closure-adjusted selected radius-4 frontier",
        "status": "cumulative5_closure_adjusted_probe_queue_emitted",
        "closures_applied": {
        ...
        "summary": {
            ...
            "cumulative5_adjusted_missing_blocks": sum(row["block_count"] for row in open_rows),
            "raw_unique_missing_serre_orbits": demand["summary"][
                "unique_missing_serre_orbits"
            ],
            "cumulative5_adjusted_unique_missing_serre_orbits": len(open_rows),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Cumulative5 Closure-Adjusted Radius-4 Frontier",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            (
                "After six verified branch-closure waves, all remaining active "
                "Serre-orbit demands except one dimension-six two-candidate orbit "
                "are single-candidate rows. The selected radius-4 frontier is now "
                "close to pure single-candidate branch closure."
            ),
            "",
        ]
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative5_adjusted_frontier.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative5_adjusted_frontier.md"
        ),
    )
