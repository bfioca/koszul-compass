#!/usr/bin/env python3
"""Exact monoid audit for mass-bound rows produced by batch-3 known-line resolution."""
...
SOURCE = REPORTS / "phenomenology_guided_q1_radius4_batch3_known_line_resolved.json"
...
def build_report() -> dict[str, Any]:
    ...
    gates = {
        "source_known_line_report_imported": gate(
            source["summary"]["statuses"].get("no_certified_triplet_mass_operator_found") == 4,
            str(SOURCE),
            "audit starts from the batch-3 known-line report with four mass-bound rows",
        ),
        "all_mass_bound_rows_audited": gate(
            len(audited) == 4,
            str(SOURCE),
            "all known-line mass-bound rows were audited",
        ),
        "all_mass_entries_obstructed": gate(
            ...
            "every mass-bound row is upgraded to a certified singlet-monoid obstruction",
        ),
    }
    return {
        "scope": "exact monoid audit for mass-bound rows from batch-3 known-line resolution",
        "status": "batch3_known_line_mass_bound_rows_upgraded_to_monoid_obstructions",
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Batch-3 Known-Line Mass Monoid Audit",
        "",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=str(
            REPORTS
            / "phenomenology_guided_q1_radius4_batch3_known_line_mass_monoid_audit.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_batch3_known_line_mass_monoid_audit.md"
        ),
    )
