#!/usr/bin/env python3
"""Cumulative selected radius-4 frontier after twelfth branch closure."""
...
from build_phenomenology_guided_q1_radius4_cumulative10_adjusted_frontier import (  # noqa: E402
    CLOSED_CANDIDATES as CUMULATIVE10_CLOSED_CANDIDATES,
    CLOSED_ORBITS as CUMULATIVE10_CLOSED_ORBITS,
)
from build_phenomenology_guided_q1_radius4_twelfth_orbit_branch_closure import (  # noqa: E402
    TARGET_CANDIDATE as TWELFTH_TARGET_CANDIDATE,
    TWELFTH_ORBIT,
)

CLOSED_CANDIDATES = set(CUMULATIVE10_CLOSED_CANDIDATES) | {TWELFTH_TARGET_CANDIDATE}
CLOSED_ORBITS = set(CUMULATIVE10_CLOSED_ORBITS) | {TWELFTH_ORBIT}
...
def build_report() -> dict[str, Any]:
    ...
    verification_paths = [
        ...
        REPORTS / "phenomenology_guided_q1_radius4_twelfth_orbit_branch_closure_verification.json",
    ]
    ...
    gates = {
        "all_closure_verifications_pass": gate(
            ...
            "cumulative11 frontier extends only verified no-viable branch closures",
        ),
        "cumulative_counts_match": gate(
            len(open_candidates) == 5
            and sum(row["block_count"] for row in open_rows) == 12
            and len(open_rows) == 6,
            "cumulative11 open candidate/orbit rows",
            "twenty-two branch-closed candidates and nineteen Serre orbits are removed",
        ),
        "open_distribution_match": gate(
            dict(sorted(open_distribution.items())) == {1: 4, 2: 1},
            "cumulative11 open candidates",
            "open candidate missing-orbit distribution matches cumulative11 subtraction",
        ),
        "next_open_orbit_match": gate(
            bool(open_rows)
            and open_rows[0]["candidate_count"] == 1
            and open_rows[0]["block_count"] == 2,
            "cumulative11 open Serre orbit ranking",
        ...
        "closed_items_absent": gate(
            ...
            "cumulative11 open frontier",
            "closed candidates and Serre orbits are absent",
        ),
    }
    return {
        "scope": "cumulative11 closure-adjusted selected radius-4 frontier",
        "status": "cumulative11_closure_adjusted_probe_queue_emitted",
        "closures_applied": {
        ...
        "summary": {
            ...
            "cumulative11_adjusted_missing_blocks": sum(row["block_count"] for row in open_rows),
            "raw_unique_missing_serre_orbits": demand["summary"][
                "unique_missing_serre_orbits"
            ],
            "cumulative11_adjusted_unique_missing_serre_orbits": len(open_rows),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Cumulative11 Closure-Adjusted Radius-4 Frontier",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            (
                "After applying the verified twelfth branch closure, all remaining "
                "active Serre-orbit demands remain single-candidate rows. The selected "
                "radius-4 frontier is reduced to five character-incomplete candidates."
            ),
            "",
        ]
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative11_adjusted_frontier.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative11_adjusted_frontier.md"
        ),
    )
