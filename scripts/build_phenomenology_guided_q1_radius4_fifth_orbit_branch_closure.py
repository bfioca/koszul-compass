#!/usr/bin/env python3
"""Branch-close the cumulative3 next dimension-2 Serre orbit."""
...
FIFTH_ORBIT = (0, -2, 0, -2, 0, 1, 0)
FIFTH_ORBIT_DUAL = tuple(-value for value in FIFTH_ORBIT)
TARGET_CANDIDATES = [
    ...
    for sector_key, sector in resolved.items():
        for cert in sector["line_certificates"]:
            if cert["actual_character_computed"] or not any(cert["cohomology"]):
                continue
            if serre_orbit(cert["line_bundle"]) != FIFTH_ORBIT:
                still_unresolved.append(
                    {
                        "sector": sector_key,
                        "line_bundle": cert["line_bundle"],
                        "cohomology": cert["cohomology"],
                        "reason": "outside_fifth_serre_orbit_branch",
                    }
                )
                continue
            actual = actual_for_line(cert["line_bundle"], cert["cohomology"], trace)
            cert["actual"] = actual
            cert["method"] = f"fifth_serre_orbit_bounded_branch_trace_{trace:+d}"
...
def build_report() -> dict[str, Any]:
    ...
    cumulative3 = load_json(REPORTS / "phenomenology_guided_q1_radius4_cumulative3_adjusted_frontier.json")
    cumulative3_verification = load_json(
        REPORTS / "phenomenology_guided_q1_radius4_cumulative3_adjusted_frontier_verification.json"
    )
    ...
    for item in targets:
        for trace in [2, 0, -2]:
            ...
            certified["fifth_serre_orbit_branch_resolution"] = resolution
            ...
            filtered = candidate_certificate_from_5259_record(
                label=f"{item['batch']}_{item['filtered']['label']}_fifth_trace_{trace:+d}",
                record=certified,
                conf=conf,
            )
            ...
            filtered["fifth_serre_orbit_branch_resolution"] = resolution
            if not certified["character_certified"]:
                filtered["classification"] = {
                    "category": "unresolved",
                    "status": "fifth_serre_orbit_branch_still_character_incomplete",
                    "reason": (
                        "the cumulative3 next Serre orbit branch was filled, but "
    ...
    next_orbit = cumulative3["summary"]["next_probe_orbit"]
    gates = {
        "cumulative3_frontier_verified": gate(
            cumulative3["all_gates_pass"]
            and cumulative3_verification["all_gates_pass"]
            and next_orbit["serre_orbit_representative"] == list(FIFTH_ORBIT),
            str(REPORTS / "phenomenology_guided_q1_radius4_cumulative3_adjusted_frontier.json"),
            "fifth-orbit branch closure targets the verified cumulative3 bottleneck",
        ),
        "target_candidates_loaded": gate(
            ...
            "both candidates touched by the cumulative3 next orbit are loaded",
        ),
        "all_trace_branches_tested": gate(
            len(branch_records) == 6
            and sum(
                record["fifth_serre_orbit_branch_resolution"]["filled_block_count"]
                for record in branch_records
            )
            == 12,
            "fifth Serre orbit branch records",
            "two candidates times three trace branches are tested",
        ),
        "classification_counts_match": gate(
            ...
            "fifth Serre orbit branch classifications",
            "all fifth-orbit branches are rejected by the q=1/phenomenology filter",
        ),
        "no_viable_branch_found": gate(
            len(viable) == 0,
            "fifth Serre orbit branch classifications",
            "no branch of the cumulative3 next orbit produces a viable candidate",
        ),
    }
    return {
        "scope": "bounded branch closure for the cumulative3 next radius-4 Serre orbit",
        "status": "no_viable_candidate_found_in_fifth_serre_orbit_branches"
        if not viable
        else "viable_candidate_found_in_fifth_serre_orbit_branches",
        "fifth_serre_orbit": {
            "representative": list(FIFTH_ORBIT),
            "dual": list(FIFTH_ORBIT_DUAL),
        ...
        "summary": {
            ...
            "remaining_unresolved_blocks": sum(
                record["fifth_serre_orbit_branch_resolution"][
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Radius-4 Fifth Serre Orbit Branch Closure",
        "",
        f"Status: `{report['status']}`",
        "",
        f"- orbit representative: `{report['fifth_serre_orbit']['representative']}`",
        f"- dual: `{report['fifth_serre_orbit']['dual']}`",
        "",
        "## Summary",
        "",
    ]
    ...
    for record in report["branch_records"]:
        res = record["fifth_serre_orbit_branch_resolution"]
        lines.append(
            "- "
            f"`{record['label']}`: "
    ...
    lines.extend(
        [
            ...
            "",
            (
                "The cumulative3 bottleneck has three one-degree dimension-two "
                "trace branches per candidate. Every branch is character-certified "
                "and nonviable: four lose the q=1 signature, and two q=1 survivors "
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_fifth_orbit_branch_closure.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_fifth_orbit_branch_closure.md"
        ),
    )
