#!/usr/bin/env python3
"""Exact monoid audit for mass-bound rows produced by batch-4 known-line resolution."""
...
SOURCE = REPORTS / "phenomenology_guided_q1_radius4_batch4_known_line_resolved.json"
...
def build_report() -> dict[str, Any]:
    ...
    for record in targets:
        ...
        all_obstructed = all(entry["certified_singlet_monoid_obstructed"] for entry in entries)
        audited.append(
            {
    ...
    gates = {
        "source_known_line_report_imported": gate(
            source["summary"]["statuses"].get("no_certified_triplet_mass_operator_found") == 1,
            str(SOURCE),
            "audit starts from the batch-4 known-line report with one mass-bound row",
        ),
        "all_mass_bound_rows_audited": gate(
            len(audited) == 1,
            str(SOURCE),
            "the single known-line mass-bound row was audited",
        ),
        "all_mass_entries_obstructed": gate(
            ...
            "the mass-bound row is upgraded to a certified singlet-monoid obstruction",
        ),
    }
    return {
        "scope": "exact monoid audit for mass-bound rows from batch-4 known-line resolution",
        "status": "batch4_known_line_mass_bound_rows_upgraded_to_monoid_obstructions",
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Batch-4 Known-Line Mass Monoid Audit",
        "",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=str(
            REPORTS
            / "phenomenology_guided_q1_radius4_batch4_known_line_mass_monoid_audit.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_batch4_known_line_mass_monoid_audit.md"
        ),
    )
