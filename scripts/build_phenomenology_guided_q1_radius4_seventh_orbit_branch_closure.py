#!/usr/bin/env python3
"""Branch-close the cumulative5 next dimension-6 Serre orbit."""
...
SEVENTH_ORBIT = (0, 0, -1, -3, 0, 1, 1)
SEVENTH_ORBIT_DUAL = tuple(-value for value in SEVENTH_ORBIT)
TARGET_CANDIDATES = [
...
def rep(trace: int) -> dict[str, Any]:
    return representation_record(6, trace)
...
def actual_for_line(line: list[int], cohomology: list[int], trace: int) -> dict[str, Any]:
    tup = tuple(line)
    if tup == SEVENTH_ORBIT:
        if cohomology != [0, 6, 0, 0]:
            raise ValueError((line, cohomology))
        return {"H1": rep(trace)}
    if tup == SEVENTH_ORBIT_DUAL:
        if cohomology != [0, 0, 6, 0]:
            raise ValueError((line, cohomology))
        return {"H2": rep(trace)}
    ...
    for sector_key, sector in resolved.items():
        for cert in sector["line_certificates"]:
            if cert["actual_character_computed"] or not any(cert["cohomology"]):
                continue
            if serre_orbit(cert["line_bundle"]) != SEVENTH_ORBIT:
                still_unresolved.append(
                    {
                        "sector": sector_key,
                        "line_bundle": cert["line_bundle"],
                        "cohomology": cert["cohomology"],
                        "reason": "outside_seventh_serre_orbit_branch",
                    }
                )
                continue
            actual = actual_for_line(cert["line_bundle"], cert["cohomology"], trace)
            cert["actual"] = actual
            cert["method"] = f"seventh_serre_orbit_bounded_branch_trace_{trace:+d}"
...
def build_report() -> dict[str, Any]:
    ...
    cumulative5 = load_json(REPORTS / "phenomenology_guided_q1_radius4_cumulative5_adjusted_frontier.json")
    cumulative5_verification = load_json(
        REPORTS / "phenomenology_guided_q1_radius4_cumulative5_adjusted_frontier_verification.json"
    )
    conf = split["full_picard_presentation_7914"]["conf"]
    targets = load_target_records()
    branch_records = []
    for item in targets:
        for trace in [6, 4, 2, 0, -2, -4, -6]:
            ...
            certified["seventh_serre_orbit_branch_resolution"] = resolution
            ...
            filtered = candidate_certificate_from_5259_record(
                label=f"{item['batch']}_{item['filtered']['label']}_seventh_trace_{trace:+d}",
                record=certified,
                conf=conf,
            )
            ...
            filtered["seventh_serre_orbit_branch_resolution"] = resolution
            if not certified["character_certified"]:
                filtered["classification"] = {
                    "category": "unresolved",
                    "status": "seventh_serre_orbit_branch_still_character_incomplete",
                    "reason": (
                        "the cumulative5 next Serre orbit branch was filled, but "
    ...
    next_orbit = cumulative5["summary"]["next_probe_orbit"]
    gates = {
        "cumulative5_frontier_verified": gate(
            cumulative5["all_gates_pass"]
            and cumulative5_verification["all_gates_pass"]
            and next_orbit["serre_orbit_representative"] == list(SEVENTH_ORBIT),
            str(REPORTS / "phenomenology_guided_q1_radius4_cumulative5_adjusted_frontier.json"),
            "seventh-orbit branch closure targets the verified cumulative5 bottleneck",
        ),
        "target_candidates_loaded": gate(
            ...
            "both candidates touched by the cumulative5 next orbit are loaded",
        ),
        "all_trace_branches_tested": gate(
            len(branch_records) == 14
            and sum(
                record["seventh_serre_orbit_branch_resolution"]["filled_block_count"]
                for record in branch_records
            )
            == 28,
            "seventh Serre orbit branch records",
            "two candidates times seven trace branches are tested",
        ),
        "classification_counts_match": gate(
            dict(sorted(categories.items()))
            == {"phenomenologically obstructed": 14}
            ...
            "seventh Serre orbit branch classifications",
            "all seventh-orbit branches are rejected by the q=1/phenomenology filter",
        ),
        "no_viable_branch_found": gate(
            len(viable) == 0,
            "seventh Serre orbit branch classifications",
            "no branch of the cumulative5 next orbit produces a viable candidate",
        ),
    }
    return {
        "scope": "bounded branch closure for the cumulative5 next radius-4 Serre orbit",
        "status": "no_viable_candidate_found_in_seventh_serre_orbit_branches"
        if not viable
        else "viable_candidate_found_in_seventh_serre_orbit_branches",
        "seventh_serre_orbit": {
            "representative": list(SEVENTH_ORBIT),
            "dual": list(SEVENTH_ORBIT_DUAL),
        ...
        "summary": {
            ...
            "remaining_unresolved_blocks": sum(
                record["seventh_serre_orbit_branch_resolution"][
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Radius-4 Seventh Serre Orbit Branch Closure",
        "",
        f"Status: `{report['status']}`",
        "",
        f"- orbit representative: `{report['seventh_serre_orbit']['representative']}`",
        f"- dual: `{report['seventh_serre_orbit']['dual']}`",
        "",
        "## Summary",
        "",
    ]
    ...
    for record in report["branch_records"]:
        res = record["seventh_serre_orbit_branch_resolution"]
        lines.append(
            "- "
            f"`{record['label']}`: "
    ...
    lines.extend(
        [
            ...
            "",
            (
                "The cumulative5 bottleneck has seven dimension-six trace branches "
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_seventh_orbit_branch_closure.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_seventh_orbit_branch_closure.md"
        ),
    )
