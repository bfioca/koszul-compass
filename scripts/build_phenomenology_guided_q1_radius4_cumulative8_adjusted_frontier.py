#!/usr/bin/env python3
"""Cumulative selected radius-4 frontier after ninth/residual branch closure."""
...
from build_phenomenology_guided_q1_radius4_cumulative7_adjusted_frontier import (  # noqa: E402
    CLOSED_CANDIDATES as CUMULATIVE7_CLOSED_CANDIDATES,
    CLOSED_ORBITS as CUMULATIVE7_CLOSED_ORBITS,
)

NINTH_TARGET_CANDIDATE = "batch2:window3_radius4_adjacency_filtered_1_known_line_resolved"
NINTH_ORBIT = (-2, 0, -3, 0, 1, 1, 0)
NINTH_RESIDUAL_ORBIT = (-1, 0, -3, 0, 0, 1, 0)

CLOSED_CANDIDATES = set(CUMULATIVE7_CLOSED_CANDIDATES) | {NINTH_TARGET_CANDIDATE}
CLOSED_ORBITS = set(CUMULATIVE7_CLOSED_ORBITS) | {NINTH_ORBIT, NINTH_RESIDUAL_ORBIT}
...
def build_report() -> dict[str, Any]:
    ...
    verification_paths = [
        ...
        REPORTS / "phenomenology_guided_q1_radius4_ninth_orbit_branch_closure_verification.json",
        REPORTS
    ...
    gates = {
        "all_closure_verifications_pass": gate(
            ...
            "cumulative8 frontier extends only verified no-viable branch closures",
        ),
        "cumulative_counts_match": gate(
            len(open_candidates) == 8
            and sum(row["block_count"] for row in open_rows) == 28
            and len(open_rows) == 14,
            "cumulative8 open candidate/orbit rows",
            "nineteen branch-closed candidates and eleven Serre orbits are removed",
        ),
        "open_distribution_match": gate(
            dict(sorted(open_distribution.items())) == {1: 6, 2: 1, 6: 1},
            "cumulative8 open candidates",
            "open candidate missing-orbit distribution matches cumulative8 subtraction",
        ),
        "next_open_orbit_match": gate(
            bool(open_rows)
            and open_rows[0]["candidate_count"] == 1
            and open_rows[0]["block_count"] == 2,
            "cumulative8 open Serre orbit ranking",
        ...
        "closed_items_absent": gate(
            ...
            "cumulative8 open frontier",
            "closed candidates and Serre orbits are absent",
        ),
    }
    return {
        "scope": "cumulative8 closure-adjusted selected radius-4 frontier",
        "status": "cumulative8_closure_adjusted_probe_queue_emitted",
        "closures_applied": {
        ...
        "summary": {
            ...
            "cumulative8_adjusted_missing_blocks": sum(row["block_count"] for row in open_rows),
            "raw_unique_missing_serre_orbits": demand["summary"][
                "unique_missing_serre_orbits"
            ],
            "cumulative8_adjusted_unique_missing_serre_orbits": len(open_rows),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Cumulative8 Closure-Adjusted Radius-4 Frontier",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            (
                "After applying the verified ninth/residual branch closure, all remaining "
                "active Serre-orbit demands remain single-candidate rows. The selected "
                "radius-4 frontier is reduced to eight character-incomplete candidates."
            ),
            "",
        ]
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative8_adjusted_frontier.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative8_adjusted_frontier.md"
        ),
    )
