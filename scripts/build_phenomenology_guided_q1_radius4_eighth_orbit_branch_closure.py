#!/usr/bin/env python3
"""Branch-close the cumulative6 next single-candidate mixed Serre orbit."""
...
EIGHTH_ORBIT = (-2, 0, -3, -1, 1, 1, 1)
EIGHTH_ORBIT_DUAL = tuple(-value for value in EIGHTH_ORBIT)
TARGET_CANDIDATE = "batch2:window3_radius4_adjacency_filtered_2_known_line_resolved"
...
def actual_for_line(
    ...
    *,
    ...
    tup = tuple(line)
    if tup == EIGHTH_ORBIT:
        if cohomology != [0, 2, 4, 0]:
            raise ValueError((line, cohomology))
        return {"H1": rep(2, h1_trace), "H2": rep(4, h2_trace)}
    if tup == EIGHTH_ORBIT_DUAL:
        if cohomology != [0, 4, 2, 0]:
            raise ValueError((line, cohomology))
        return {"H1": rep(4, h2_trace), "H2": rep(2, h1_trace)}
    ...
    for sector_key, sector in resolved.items():
        for cert in sector["line_certificates"]:
            if cert["actual_character_computed"] or not any(cert["cohomology"]):
                continue
            if serre_orbit(cert["line_bundle"]) != EIGHTH_ORBIT:
                still_unresolved.append(
                    {
                        "sector": sector_key,
                        "line_bundle": cert["line_bundle"],
                        "cohomology": cert["cohomology"],
                        "reason": "outside_eighth_serre_orbit_branch",
                    }
                )
                continue
            ...
            cert["actual"] = actual
            cert["method"] = (
                "eighth_serre_orbit_bounded_branch_"
...
def load_target_record() -> dict[str, Any]:
    ...
    for resolved, filtered in zip(report["resolved_records"], report["filtered_candidate_records"]):
        cid = f"batch2:{filtered['label']}"
        if cid == TARGET_CANDIDATE:
...
def build_report() -> dict[str, Any]:
    ...
    cumulative6 = load_json(REPORTS / "phenomenology_guided_q1_radius4_cumulative6_adjusted_frontier.json")
    cumulative6_verification = load_json(
        REPORTS / "phenomenology_guided_q1_radius4_cumulative6_adjusted_frontier_verification.json"
    )
    conf = split["full_picard_presentation_7914"]["conf"]
    target = load_target_record()
    branch_records = []
    for h1_trace in [2, 0, -2]:
        for h2_trace in [4, 2, 0, -2, -4]:
            ...
            certified["eighth_serre_orbit_branch_resolution"] = resolution
            ...
            filtered = candidate_certificate_from_5259_record(
                label=(
                    ...
                    f"_eighth_h1_{h1_trace:+d}_h2_{h2_trace:+d}"
                ),
                record=certified,
                conf=conf,
            )
            ...
            filtered["eighth_serre_orbit_branch_resolution"] = resolution
            if not certified["character_certified"]:
                filtered["classification"] = {
                    "category": "unresolved",
                    "status": "eighth_serre_orbit_branch_still_character_incomplete",
                    "reason": (
                        "the cumulative6 next Serre orbit branch was filled, but "
    ...
    next_orbit = cumulative6["summary"]["next_probe_orbit"]
    gates = {
        "cumulative6_frontier_verified": gate(
            cumulative6["all_gates_pass"]
            and cumulative6_verification["all_gates_pass"]
            and next_orbit["serre_orbit_representative"] == list(EIGHTH_ORBIT),
            str(REPORTS / "phenomenology_guided_q1_radius4_cumulative6_adjusted_frontier.json"),
            "eighth-orbit branch closure targets the verified cumulative6 bottleneck",
        ),
        "target_candidate_loaded": gate(
            f"batch2:{target['filtered']['label']}" == TARGET_CANDIDATE,
            str(SOURCE_REPORT),
            "the single candidate touched by the cumulative6 next orbit is loaded",
        ),
        "all_trace_branches_tested": gate(
            len(branch_records) == 15
            and sum(
                record["eighth_serre_orbit_branch_resolution"]["filled_block_count"]
                for record in branch_records
            )
            == 30,
            "eighth Serre orbit branch records",
            "one candidate times fifteen H1/H2 trace branches are tested",
        ),
        "classification_counts_match": gate(
            ...
            == {
                ...
                "rejected_spectrum_signature_not_q1_three_family": 14,
            },
            "eighth Serre orbit branch classifications",
            "all eighth-orbit branches are rejected by the q=1/phenomenology filter",
        ),
        "no_viable_branch_found": gate(
            len(viable) == 0,
            "eighth Serre orbit branch classifications",
            "no branch of the cumulative6 next orbit produces a viable candidate",
        ),
    }
    return {
        "scope": "bounded branch closure for the cumulative6 next radius-4 Serre orbit",
        "status": "no_viable_candidate_found_in_eighth_serre_orbit_branches"
        if not viable
        else "viable_candidate_found_in_eighth_serre_orbit_branches",
        "eighth_serre_orbit": {
            "representative": list(EIGHTH_ORBIT),
            "dual": list(EIGHTH_ORBIT_DUAL),
        ...
        "summary": {
            ...
            "remaining_unresolved_blocks": sum(
                record["eighth_serre_orbit_branch_resolution"][
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Radius-4 Eighth Serre Orbit Branch Closure",
        "",
        f"Status: `{report['status']}`",
        "",
        f"- orbit representative: `{report['eighth_serre_orbit']['representative']}`",
        f"- dual: `{report['eighth_serre_orbit']['dual']}`",
        f"- target candidate: `{report['eighth_serre_orbit']['target_candidate']}`",
        "",
        "## Summary",
        "",
    ]
    ...
    for record in report["branch_records"]:
        res = record["eighth_serre_orbit_branch_resolution"]
        lines.append(
            "- "
            f"`{record['label']}`: "
    ...
    lines.extend(
        [
            ...
            "",
            (
                "The cumulative6 bottleneck has fifteen mixed-degree trace branches. "
                "Every branch is character-certified and nonviable under the "
                "q=1 spectrum and charge-level filters."
            ),
            "",
        ]
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_eighth_orbit_branch_closure.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_eighth_orbit_branch_closure.md"
        ),
    )
