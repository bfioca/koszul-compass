#!/usr/bin/env python3
"""Verify the adjusted next Serre-orbit branch closure report."""
...
def verify() -> dict[str, Any]:
    path = REPORTS / "phenomenology_guided_q1_radius4_next_orbit_branch_closure.json"
    md_path = REPORTS / "phenomenology_guided_q1_radius4_next_orbit_branch_closure.md"
    ...
    gates = {
        "builder_gates_pass": gate(
            ...
            str(path),
            "builder-side next-orbit branch gates passed",
        ),
        "target_orbit_matches_adjusted_frontier": gate(
            report["next_serre_orbit"]["representative"] == [-1, 0, 2, 0, 2, -1, 0]
            and report["next_serre_orbit"]["dual"] == [1, 0, -2, 0, -2, 1, 0]
            and report["next_serre_orbit"]["target_candidates"]
            == [
            ...
            str(path),
            "branch closure targets the verified adjusted next Serre orbit",
        ),
        ...
        "classification_counts_match": gate(
            ...
            str(path),
            "all next-orbit branches are nonviable with exact statuses",
        ),
        "no_viable_branch_found": gate(
            report["summary"]["viable_count"] == len(viable) == 0
            and report["status"] == "no_viable_candidate_found_in_next_serre_orbit_branches",
            str(path),
            "no H1/H2 trace branch of the adjusted next orbit is viable",
        ),
        "markdown_exposes_closure": gate(
            ...
            str(md_path),
            "markdown exposes next-orbit branch count and obstruction statuses",
        ),
    }
    return {
        "scope": "verification for adjusted next Serre-orbit branch closure",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=str(
            REPORTS
            / "phenomenology_guided_q1_radius4_next_orbit_branch_closure_verification.json"
        ),
    )
