#!/usr/bin/env python3
"""Cumulative selected radius-4 frontier after thirteenth branch closure."""
...
from build_phenomenology_guided_q1_radius4_cumulative11_adjusted_frontier import (  # noqa: E402
    CLOSED_CANDIDATES as CUMULATIVE11_CLOSED_CANDIDATES,
    CLOSED_ORBITS as CUMULATIVE11_CLOSED_ORBITS,
)
from build_phenomenology_guided_q1_radius4_thirteenth_orbit_branch_closure import (  # noqa: E402
    TARGET_CANDIDATE as THIRTEENTH_TARGET_CANDIDATE,
    THIRTEENTH_ORBIT,
)

CLOSED_CANDIDATES = set(CUMULATIVE11_CLOSED_CANDIDATES) | {THIRTEENTH_TARGET_CANDIDATE}
CLOSED_ORBITS = set(CUMULATIVE11_CLOSED_ORBITS) | {THIRTEENTH_ORBIT}
...
def build_report() -> dict[str, Any]:
    ...
    verification_paths = [
        ...
        REPORTS / "phenomenology_guided_q1_radius4_thirteenth_orbit_branch_closure_verification.json",
    ]
    ...
    gates = {
        "all_closure_verifications_pass": gate(
            ...
            "cumulative12 frontier extends only verified no-viable branch closures",
        ),
        "cumulative_counts_match": gate(
            len(open_candidates) == 4
            and sum(row["block_count"] for row in open_rows) == 10
            and len(open_rows) == 5,
            "cumulative12 open candidate/orbit rows",
            "twenty-three branch-closed candidates and twenty Serre orbits are removed",
        ),
        "open_distribution_match": gate(
            dict(sorted(open_distribution.items())) == {1: 3, 2: 1},
            "cumulative12 open candidates",
            "open candidate missing-orbit distribution matches cumulative12 subtraction",
        ),
        "next_open_orbit_match": gate(
            bool(open_rows)
            and open_rows[0]["candidate_count"] == 1
            and open_rows[0]["block_count"] == 2,
            "cumulative12 open Serre orbit ranking",
        ...
        "closed_items_absent": gate(
            ...
            "cumulative12 open frontier",
            "closed candidates and Serre orbits are absent",
        ),
    }
    return {
        "scope": "cumulative12 closure-adjusted selected radius-4 frontier",
        "status": "cumulative12_closure_adjusted_probe_queue_emitted",
        "closures_applied": {
        ...
        "summary": {
            ...
            "cumulative12_adjusted_missing_blocks": sum(row["block_count"] for row in open_rows),
            "raw_unique_missing_serre_orbits": demand["summary"][
                "unique_missing_serre_orbits"
            ],
            "cumulative12_adjusted_unique_missing_serre_orbits": len(open_rows),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Cumulative12 Closure-Adjusted Radius-4 Frontier",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            (
                "After applying the verified thirteenth branch closure, all remaining "
                "active Serre-orbit demands remain single-candidate rows. The selected "
                "radius-4 frontier is reduced to four character-incomplete candidates."
            ),
            "",
        ]
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative12_adjusted_frontier.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative12_adjusted_frontier.md"
        ),
    )
