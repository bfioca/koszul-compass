#!/usr/bin/env python3
"""Cumulative selected radius-4 frontier after fourteenth cluster closure."""
...
from build_phenomenology_guided_q1_radius4_cumulative12_adjusted_frontier import (  # noqa: E402
    CLOSED_CANDIDATES as CUMULATIVE12_CLOSED_CANDIDATES,
    CLOSED_ORBITS as CUMULATIVE12_CLOSED_ORBITS,
)
from build_phenomenology_guided_q1_radius4_fourteenth_cluster_branch_closure import (  # noqa: E402
    CLUSTER_ORBITS,
    TARGET_CANDIDATE as FOURTEENTH_CLUSTER_TARGET_CANDIDATE,
)

CLOSED_CANDIDATES = set(CUMULATIVE12_CLOSED_CANDIDATES) | {
    FOURTEENTH_CLUSTER_TARGET_CANDIDATE
}
CLOSED_ORBITS = set(CUMULATIVE12_CLOSED_ORBITS) | set(CLUSTER_ORBITS)
...
def build_report() -> dict[str, Any]:
    ...
    gates = {
        "all_closure_verifications_pass": gate(
            ...
            "cumulative13 frontier extends only verified no-viable branch closures",
        ),
        "cumulative_counts_match": gate(
            len(open_candidates) == 3
            and sum(row["block_count"] for row in open_rows) == 6
            and len(open_rows) == 3,
            "cumulative13 open candidate/orbit rows",
            "twenty-four branch-closed candidates and twenty-two Serre orbits are removed",
        ),
        "open_distribution_match": gate(
            dict(sorted(open_distribution.items())) == {1: 3},
            "cumulative13 open candidates",
            "open candidate missing-orbit distribution matches cumulative13 subtraction",
        ),
        "next_open_orbit_match": gate(
            bool(open_rows)
            and open_rows[0]["candidate_count"] == 1
            and open_rows[0]["block_count"] == 2,
            "cumulative13 open Serre orbit ranking",
        ...
        "closed_items_absent": gate(
            ...
            "cumulative13 open frontier",
            "closed candidates and Serre orbits are absent",
        ),
    }
    return {
        "scope": "cumulative13 closure-adjusted selected radius-4 frontier",
        "status": "cumulative13_closure_adjusted_probe_queue_emitted",
        "closures_applied": {
        ...
        "summary": {
            ...
            "cumulative13_adjusted_missing_blocks": sum(row["block_count"] for row in open_rows),
            "raw_unique_missing_serre_orbits": demand["summary"][
                "unique_missing_serre_orbits"
            ],
            "cumulative13_adjusted_unique_missing_serre_orbits": len(open_rows),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Cumulative13 Closure-Adjusted Radius-4 Frontier",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            (
                "After applying the verified fourteenth two-orbit cluster closure, "
                "the selected radius-4 frontier is reduced to three character-incomplete "
...
def main() -> int:
    ...
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative13_adjusted_frontier.md"
        ),
    )
