#!/usr/bin/env python3
"""Cumulative selected radius-4 frontier after four verified branch closures."""
...
def build_report() -> dict[str, Any]:
    ...
    gates = {
        "all_closure_verifications_pass": gate(
            ...
            "cumulative3 frontier subtracts only verified no-viable branch closures",
        ),
        "cumulative_counts_match": gate(
            len(open_candidates) == 16
            and sum(row["block_count"] for row in open_rows) == 46
            and len(open_rows) == 20,
            "cumulative3 open candidate/orbit rows",
            "eleven branch-closed candidates and five Serre orbits are removed",
        ),
        "open_distribution_match": gate(
            dict(sorted(open_distribution.items())) == {1: 13, 2: 2, 6: 1},
            "cumulative3 open candidates",
            "open candidate missing-orbit distribution matches cumulative3 subtraction",
        ),
        "next_open_orbit_match": gate(
            open_rows[0]["serre_orbit_representative"] == [0, -2, 0, -2, 0, 1, 0]
            and open_rows[0]["candidate_count"] == 2
            and open_rows[0]["block_count"] == 4,
            "cumulative3 open Serre orbit ranking",
            "next cumulative3 bottleneck is the remaining two-candidate single-degree orbit",
        ),
        "closed_items_absent": gate(
            ...
            "cumulative3 open frontier",
            "closed candidates and Serre orbits are absent",
        ),
    }
    return {
        "scope": "cumulative3 closure-adjusted selected radius-4 frontier",
        "status": "cumulative3_closure_adjusted_probe_queue_emitted",
        "closures_applied": {
        ...
        "summary": {
            ...
            "cumulative3_adjusted_missing_blocks": sum(row["block_count"] for row in open_rows),
            "raw_unique_missing_serre_orbits": demand["summary"][
                "unique_missing_serre_orbits"
            ],
            "cumulative3_adjusted_unique_missing_serre_orbits": len(open_rows),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Cumulative3 Closure-Adjusted Radius-4 Frontier",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            (
                "After four verified branch-closure waves, all active multi-candidate "
                "Serre-orbit bottlenecks except one single-degree dimension-two orbit "
                "are closed. The next active target is the leading two-candidate "
                "Serre orbit listed above."
            ),
            "",
        ]
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative3_adjusted_frontier.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative3_adjusted_frontier.md"
        ),
    )
