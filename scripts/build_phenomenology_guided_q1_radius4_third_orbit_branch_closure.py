#!/usr/bin/env python3
"""Branch-close the current cumulative next Serre orbit in radius-4 frontier."""
...
THIRD_ORBIT = (-1, 0, 2, 1, 1, -1, 0)
THIRD_ORBIT_DUAL = tuple(-value for value in THIRD_ORBIT)
TARGET_CANDIDATES = [
    ...
    for sector_key, sector in resolved.items():
        for cert in sector["line_certificates"]:
            if cert["actual_character_computed"] or not any(cert["cohomology"]):
                continue
            if serre_orbit(cert["line_bundle"]) != THIRD_ORBIT:
                still_unresolved.append(
                    {
                        "sector": sector_key,
                        "line_bundle": cert["line_bundle"],
                        "cohomology": cert["cohomology"],
                        "reason": "outside_third_serre_orbit_branch",
                    }
                )
                continue
            ...
            cert["actual"] = actual
            cert["method"] = (
                "third_serre_orbit_bounded_branch_"
...
def build_report() -> dict[str, Any]:
    ...
    cumulative = load_json(REPORTS / "phenomenology_guided_q1_radius4_cumulative_adjusted_frontier.json")
    cumulative_verification = load_json(
        REPORTS / "phenomenology_guided_q1_radius4_cumulative_adjusted_frontier_verification.json"
    )
    ...
    for item in targets:
        for h1_trace in [2, 0, -2]:
            for h2_trace in [2, 0, -2]:
                ...
                certified["third_serre_orbit_branch_resolution"] = resolution
                ...
                filtered = candidate_certificate_from_5259_record(
                    label=(
                        ...
                        f"_third_h1_{h1_trace:+d}_h2_{h2_trace:+d}"
                    ),
                    record=certified,
                    conf=conf,
                )
                ...
                filtered["third_serre_orbit_branch_resolution"] = resolution
                if not certified["character_certified"]:
                    filtered["classification"] = {
                        "category": "unresolved",
                        "status": "third_serre_orbit_branch_still_character_incomplete",
                        "reason": (
                            "the cumulative next Serre orbit branch was filled, but "
    ...
    next_orbit = cumulative["summary"]["next_probe_orbit"]
    gates = {
        "cumulative_frontier_verified": gate(
            cumulative["all_gates_pass"]
            and cumulative_verification["all_gates_pass"]
            and next_orbit["serre_orbit_representative"] == list(THIRD_ORBIT),
            str(REPORTS / "phenomenology_guided_q1_radius4_cumulative_adjusted_frontier.json"),
            "third-orbit branch closure targets the verified cumulative bottleneck",
        ),
        "target_candidates_loaded": gate(
            ...
            "both candidates touched by the cumulative next orbit are loaded",
        ),
        "all_trace_branches_tested": gate(
            len(branch_records) == 18
            and sum(
                record["third_serre_orbit_branch_resolution"]["filled_block_count"]
                for record in branch_records
            )
            == 36,
            "third Serre orbit branch records",
        ...
        "classification_counts_match": gate(
            ...
            "third Serre orbit branch classifications",
            "all third-orbit branches are rejected by the q=1/phenomenology filter",
        ),
        "no_viable_branch_found": gate(
            len(viable) == 0,
            "third Serre orbit branch classifications",
            "no branch of the cumulative next orbit produces a viable candidate",
        ),
    }
    return {
        "scope": "bounded branch closure for the cumulative next radius-4 Serre orbit",
        "status": "no_viable_candidate_found_in_third_serre_orbit_branches"
        if not viable
        else "viable_candidate_found_in_third_serre_orbit_branches",
        "third_serre_orbit": {
            "representative": list(THIRD_ORBIT),
            "dual": list(THIRD_ORBIT_DUAL),
        ...
        "summary": {
            ...
            "remaining_unresolved_blocks": sum(
                record["third_serre_orbit_branch_resolution"][
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Radius-4 Third Serre Orbit Branch Closure",
        "",
        f"Status: `{report['status']}`",
        "",
        f"- orbit representative: `{report['third_serre_orbit']['representative']}`",
        f"- dual: `{report['third_serre_orbit']['dual']}`",
        "",
        "## Summary",
        "",
    ]
    ...
    for record in report["branch_records"]:
        res = record["third_serre_orbit_branch_resolution"]
        lines.append(
            "- "
            f"`{record['label']}`: "
    ...
    lines.extend(
        [
            ...
            "",
            (
                "The current cumulative bottleneck has nine Serre-compatible "
                "H1/H2 trace branches per candidate. Every branch is character-certified "
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_third_orbit_branch_closure.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_third_orbit_branch_closure.md"
        ),
    )
