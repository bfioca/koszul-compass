#!/usr/bin/env python3
"""Summarize batch-4 frontier after branch analysis and exact mass closure."""
...
def build_report() -> dict[str, Any]:
    ...
    branch = load_json(REPORTS / "phenomenology_guided_q1_radius4_batch4_branch_analysis.json")
    mass = load_json(
        REPORTS / "phenomenology_guided_q1_radius4_batch4_branch_mass_monoid_audit.json"
    )
    ...
    gates = {
        "imports_batch4_known_line": gate(
            ...
            "batch4 starts with 31 known-line-incomplete q=1 survivor records",
        ),
        "imports_branch_analysis": gate(
            branch["all_gates_pass"]
            and branch["summary"]["unresolved_records"] == 31
            and branch["summary"]["records_skipped"] == 0
            and branch["summary"]["viable_branches"] == 0,
            str(REPORTS / "phenomenology_guided_q1_radius4_batch4_branch_analysis.json"),
            "all batch4 incomplete records were branch-enumerated with no viable branch",
        ),
        "imports_exact_mass_closure": gate(
            mass["all_gates_pass"]
            and mass["summary"]["mass_bound_branches"] == 9
            and mass["summary"]["open_mass_bound_uncertainties"] == 0,
            str(REPORTS / "phenomenology_guided_q1_radius4_batch4_branch_mass_monoid_audit.json"),
        ...
        "adjusted_q1_statuses_have_no_unresolved_mass_bound": gate(
            ...
            "batch4 adjusted branch status accounting",
            "branch statuses are adjusted by the exact mass-monoid closure",
        ),
    }
    return {
        "scope": "batch4 branch-closed selected radius-4 q=1 frontier",
        "status": "batch4_known_line_incomplete_frontier_branch_closed_no_viable_candidate",
        "summary": {
            "known_line_incomplete_records_closed": 31,
        ...
        "interpretation": (
            "Every batch4 known-line-incomplete survivor has been closed by bounded "
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Batch-4 Branch-Closed Frontier",
        "",
...
def main() -> int:
    ...
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_batch4_branch_closed_frontier.md"
        ),
    )
