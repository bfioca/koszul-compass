#!/usr/bin/env python3
"""Bounded branch analysis for batch-4 known-line-incomplete q=1 records."""
...
def complete_characters_with_branch(
    ...
    for block, actual in zip(blocks, branch_actuals):
        ...
        cert["method"] = "batch4_dimension_compatible_branch_completion"
...
def source_key(record: dict[str, Any]) -> tuple[str, str]:
    return (record["source_window"], record["source_filtered_label"])
...
def certificate_from_completed_record(
    *,
    ...
    return {
        "label": label,
        "source": "batch4_known_line_incomplete_branch_completion",
...
def build_report(*, max_branches_per_record: int) -> dict[str, Any]:
    ...
    known = load_json(REPORTS / "phenomenology_guided_q1_radius4_batch4_known_line_resolved.json")
    conf = split["full_picard_presentation_7914"]["conf"]
    status_by_key = {
        source_key(record): record["classification"]["status"]
        for record in known["filtered_candidate_records"]
    }
    unresolved_records = [
        record
        for record in known["resolved_records"]
        if status_by_key[source_key(record)] == "known_line_resolution_still_incomplete"
    ]
    ...
    for record in unresolved_records:
        ...
        for branch_index, branch_actuals in enumerate(itertools.product(*option_lists)):
            ...
            completed["batch4_unresolved_branch_completion"] = {
            ...
            branch_label = (
                f"batch4_{record['source_window']}_{record['source_filtered_label']}"
            ...
            if desired_q1:
                ...
                filtered["batch4_unresolved_branch_completion"] = completed[
                    "batch4_unresolved_branch_completion"
                ]
    ...
    status = (
        "viable_branch_found_in_batch4_bounded_branch_analysis"
        if viable_count
        else "no_viable_branch_found_in_batch4_bounded_branch_analysis"
    )
    gates = {
        "imports_batch4_known_line_report": gate(
            known["all_gates_pass"]
            and known["summary"]["statuses"].get("known_line_resolution_still_incomplete") == 31,
            str(REPORTS / "phenomenology_guided_q1_radius4_batch4_known_line_resolved.json"),
            "branch analysis starts from the verified batch-4 known-line-incomplete pool",
        ),
        "all_batch4_incomplete_records_considered": gate(
            len(unresolved_records) == len(record_reports) == 31,
            str(REPORTS / "phenomenology_guided_q1_radius4_batch4_known_line_resolved.json"),
            "all batch-4 known-line-incomplete records are included",
        ),
        "bounded_records_evaluated": gate(
            ...
            "dimension-compatible batch-4 branch enumeration",
        ...
        "desired_q1_certificates_have_tables": gate(
            ...
            "batch-4 desired-q1 branch certificates",
            "all desired-q1 branches carry spectrum, character, mass, and proton tables",
        ),
    }
    return {
        "scope": "bounded dimension-compatible branch analysis for batch-4 known-line-incomplete records",
        "status": status,
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines.extend(
        [
            ...
            "",
            (
                "This batch-4 pass enumerates every dimension-compatible character "
...
def main() -> int:
    ...
    parser.add_argument("--max-branches-per-record", type=int, default=25000)
    parser.add_argument(
        "--json-out",
        default=str(REPORTS / "phenomenology_guided_q1_radius4_batch4_branch_analysis.json"),
    )
    parser.add_argument(
        "--md-out",
        default=str(REPORTS / "phenomenology_guided_q1_radius4_batch4_branch_analysis.md"),
    )
