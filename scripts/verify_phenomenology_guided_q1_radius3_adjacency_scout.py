#!/usr/bin/env python3
"""Verify the bounded radius-3 adjacency scout."""
...
def verify(*, report_path: Path, md_path: Path) -> dict[str, Any]:
    ...
    gates = {
        ...
        "adjacency_edge_count": gate(
            report["adjacency_counters"]["source_radius2_records"] == 29
            and report["adjacency_counters"]["primitive_count"] == 560
            and report["adjacency_counters"]["raw_edges"] == 16240
            and report["adjacency_counters"]["unique_candidates"] > 0,
            str(path),
            "scout generated one primitive adjacency edge from each radius-2 q=1 record",
        ),
        ...
        "markdown_exposes_scope": gate(
            "Status:" in md_text
            and "bounded one-move expansion" in md_text
    ...
    return {
        "scope": "verification for bounded radius-3 adjacency q=1 scout",
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(REPORTS / "phenomenology_guided_q1_radius3_adjacency_scout_verification.json"),
    )
