#!/usr/bin/env python3
"""Bounded branch analysis for batch-3 known-line-incomplete q=1 records."""
...
def complete_characters_with_branch(
    ...
    for block, actual in zip(blocks, branch_actuals):
        ...
        cert["method"] = "batch3_dimension_compatible_branch_completion"
...
def certificate_from_completed_record(
    *,
    ...
    return {
        "label": label,
        "source": "batch3_known_line_incomplete_branch_completion",
...
def build_report(*, max_branches_per_record: int) -> dict[str, Any]:
    ...
    for record in unresolved_records:
        ...
        for branch_index, branch_actuals in enumerate(itertools.product(*option_lists)):
            ...
            completed["batch3_unresolved_branch_completion"] = {
            ...
            branch_label = (
                f"batch3_{record['source_window']}_{record['source_filtered_label']}"
            ...
            if desired_q1:
                ...
                filtered["batch3_unresolved_branch_completion"] = completed[
                    "batch3_unresolved_branch_completion"
                ]
    ...
    status = (
        "viable_branch_found_in_batch3_bounded_branch_analysis"
        if viable_count
        else "no_viable_branch_found_in_batch3_bounded_branch_analysis"
    )
    gates = {
        "imports_batch3_known_line_report": gate(
            known["all_gates_pass"]
            and known["summary"]["statuses"].get("known_line_resolution_still_incomplete") == 13,
            str(REPORTS / "phenomenology_guided_q1_radius4_batch3_known_line_resolved.json"),
            "branch analysis starts from the verified batch-3 known-line-incomplete pool",
        ),
        "all_batch3_incomplete_records_considered": gate(
            len(unresolved_records) == len(record_reports) == 13,
            str(REPORTS / "phenomenology_guided_q1_radius4_batch3_known_line_resolved.json"),
            "all batch-3 known-line-incomplete records are included",
        ),
        "bounded_records_evaluated": gate(
            ...
            "dimension-compatible batch-3 branch enumeration",
        ...
        "desired_q1_certificates_have_tables": gate(
            ...
            "batch-3 desired-q1 branch certificates",
            "all desired-q1 branches carry spectrum, character, mass, and proton tables",
        ),
    }
    return {
        "scope": "bounded dimension-compatible branch analysis for batch-3 known-line-incomplete records",
        "status": status,
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines.extend(
        [
            ...
            "",
            (
                "This batch-3 pass enumerates every dimension-compatible character "
...
def main() -> int:
    ...
    parser.add_argument("--max-branches-per-record", type=int, default=1000)
    parser.add_argument(
        "--json-out",
        default=str(REPORTS / "phenomenology_guided_q1_radius4_batch3_branch_analysis.json"),
    )
    parser.add_argument(
        "--md-out",
        default=str(REPORTS / "phenomenology_guided_q1_radius4_batch3_branch_analysis.md"),
    )
