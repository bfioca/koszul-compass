#!/usr/bin/env python3
"""Cumulative selected radius-4 frontier after fifteenth branch closure."""
...
from build_phenomenology_guided_q1_radius4_cumulative13_adjusted_frontier import (  # noqa: E402
    CLOSED_CANDIDATES as CUMULATIVE13_CLOSED_CANDIDATES,
    CLOSED_ORBITS as CUMULATIVE13_CLOSED_ORBITS,
)
from build_phenomenology_guided_q1_radius4_fifteenth_orbit_branch_closure import (  # noqa: E402
    FIFTEENTH_ORBIT,
    TARGET_CANDIDATE as FIFTEENTH_TARGET_CANDIDATE,
)

CLOSED_CANDIDATES = set(CUMULATIVE13_CLOSED_CANDIDATES) | {FIFTEENTH_TARGET_CANDIDATE}
CLOSED_ORBITS = set(CUMULATIVE13_CLOSED_ORBITS) | {FIFTEENTH_ORBIT}
...
def build_report() -> dict[str, Any]:
    ...
    verification_paths = [
        ...
        REPORTS / "phenomenology_guided_q1_radius4_fifteenth_orbit_branch_closure_verification.json",
    ]
    ...
    gates = {
        "all_closure_verifications_pass": gate(
            ...
            "cumulative14 frontier extends only verified no-viable branch closures",
        ),
        "cumulative_counts_match": gate(
            len(open_candidates) == 2
            and sum(row["block_count"] for row in open_rows) == 4
            and len(open_rows) == 2,
            "cumulative14 open candidate/orbit rows",
            "twenty-five branch-closed candidates and twenty-three Serre orbits are removed",
        ),
        "open_distribution_match": gate(
            dict(sorted(open_distribution.items())) == {1: 2},
            "cumulative14 open candidates",
            "open candidate missing-orbit distribution matches cumulative14 subtraction",
        ),
        "next_open_orbit_match": gate(
            bool(open_rows)
            and open_rows[0]["candidate_count"] == 1
            and open_rows[0]["block_count"] == 2,
            "cumulative14 open Serre orbit ranking",
        ...
        "closed_items_absent": gate(
            ...
            "cumulative14 open frontier",
            "closed candidates and Serre orbits are absent",
        ),
    }
    return {
        "scope": "cumulative14 closure-adjusted selected radius-4 frontier",
        "status": "cumulative14_closure_adjusted_probe_queue_emitted",
        "closures_applied": {
        ...
        "summary": {
            ...
            "cumulative14_adjusted_missing_blocks": sum(row["block_count"] for row in open_rows),
            "raw_unique_missing_serre_orbits": demand["summary"][
                "unique_missing_serre_orbits"
            ],
            "cumulative14_adjusted_unique_missing_serre_orbits": len(open_rows),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Cumulative14 Closure-Adjusted Radius-4 Frontier",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            (
                "After applying the verified fifteenth branch closure, the selected "
                "radius-4 frontier is reduced to two character-incomplete "
                "single-orbit candidates."
            ),
            "",
        ]
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative14_adjusted_frontier.md"
        ),
    )
