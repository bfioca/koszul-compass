#!/usr/bin/env python3
"""Apply low repeated-pattern E2 resolutions and rerun the filter."""
    ...
    for sector_key, sector in resolved.items():
        for cert in sector["line_certificates"]:
            ...
            if not cert["actual_character_computed"] and line in actuals:
                ...
                cert["method"] = "radius3_low_repeated_e2_probe"
                ...
                unresolved.append(
                    {
                        ...
                        "reason": "no_low_repeated_e2_resolution_for_this_line",
                    }
                )
...
def build_report() -> dict[str, Any]:
    ...
    probe = load_json(REPORTS / "phenomenology_guided_q1_radius3_low_repeated_e2_probe.json")
    conf = split["full_picard_presentation_7914"]["conf"]
    actuals = {tuple(record["line_bundle"]): record["actual"] for record in probe["records"]}
    items = probe["fully_covered_low_records"]
    ...
    for item in items:
        ...
        resolved["radius3_low_repeated_resolution"] = resolution
        ...
        filtered = candidate_certificate_from_5259_record(
            label=f"{item['window']}_{item['label']}_low_repeated_rank_resolved",
            record=resolved,
            conf=conf,
        )
        filtered["source_window"] = item["window"]
        filtered["source_filtered_label"] = item["label"]
        filtered["radius3_low_repeated_resolution"] = resolution
    ...
    gates = {
        "imports_verified_probe": gate(
            ...
            str(REPORTS / "phenomenology_guided_q1_radius3_low_repeated_e2_probe.json"),
            "rank-resolved classifier imports the verified low repeated E2 probe",
        ),
        "all_fully_covered_records_attempted": gate(
            len(items) == 6
            and len(rank_resolved_records) == 6
            and len(filtered_records) == 6,
            str(REPORTS / "phenomenology_guided_q1_radius3_low_repeated_e2_probe.json"),
            "all six fully covered low-priority records were attempted",
        ),
        "all_missing_blocks_filled": gate(
            sum(record["radius3_low_repeated_resolution"]["filled_block_count"] for record in rank_resolved_records)
            == 24
            and sum(record["radius3_low_repeated_resolution"]["unresolved_block_count"] for record in rank_resolved_records)
            == 0
            and all(record["character_certified"] for record in rank_resolved_records),
            "low repeated character resolutions",
            "all twenty-four missing line blocks were filled and all six records are character-certified",
        ),
        "no_viable_low_repeated_survivor": gate(
            all(record["classification"]["category"] != "viable" for record in filtered_records),
            "low repeated phenomenology filter",
            "no low repeated resolved record is currently viable",
        ),
        "required_sections_exist": gate(
            ...
            "low repeated filtered certificates",
            "every low repeated filtered record emits the required deliverable sections",
        ),
    }
    return {
        "scope": "low-priority radius-3 repeated-pattern rank-resolved filter pass",
        "status": "no_viable_candidate_found_after_low_repeated_resolution",
        "summary": {
            "records": len(rank_resolved_records),
            "filled_blocks": sum(
                record["radius3_low_repeated_resolution"]["filled_block_count"]
                for record in rank_resolved_records
            ),
            "remaining_unresolved_blocks": sum(
                record["radius3_low_repeated_resolution"]["unresolved_block_count"]
                for record in rank_resolved_records
            ),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Radius-3 Low Repeated Rank-Resolved Records",
        "",
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(REPORTS / "phenomenology_guided_q1_radius3_low_repeated_rank_resolved.json"),
    )
    parser.add_argument(
        "--md-out",
        default=str(REPORTS / "phenomenology_guided_q1_radius3_low_repeated_rank_resolved.md"),
    )
