#!/usr/bin/env python3
"""Cumulative selected radius-4 frontier after sixteenth branch closure."""
...
from build_phenomenology_guided_q1_radius4_cumulative14_adjusted_frontier import (  # noqa: E402
    CLOSED_CANDIDATES as CUMULATIVE14_CLOSED_CANDIDATES,
    CLOSED_ORBITS as CUMULATIVE14_CLOSED_ORBITS,
)
from build_phenomenology_guided_q1_radius4_sixteenth_orbit_branch_closure import (  # noqa: E402
    SIXTEENTH_ORBIT,
    TARGET_CANDIDATE as SIXTEENTH_TARGET_CANDIDATE,
)

CLOSED_CANDIDATES = set(CUMULATIVE14_CLOSED_CANDIDATES) | {SIXTEENTH_TARGET_CANDIDATE}
CLOSED_ORBITS = set(CUMULATIVE14_CLOSED_ORBITS) | {SIXTEENTH_ORBIT}
...
def build_report() -> dict[str, Any]:
    ...
    verification_paths = [
        ...
        REPORTS / "phenomenology_guided_q1_radius4_sixteenth_orbit_branch_closure_verification.json",
    ]
    ...
    gates = {
        "all_closure_verifications_pass": gate(
            ...
            "cumulative15 frontier extends only verified no-viable branch closures",
        ),
        "cumulative_counts_match": gate(
            len(open_candidates) == 1
            and sum(row["block_count"] for row in open_rows) == 2
            and len(open_rows) == 1,
            "cumulative15 open candidate/orbit rows",
            "twenty-six branch-closed candidates and twenty-four Serre orbits are removed",
        ),
        "open_distribution_match": gate(
            dict(sorted(open_distribution.items())) == {1: 1},
            "cumulative15 open candidates",
            "open candidate missing-orbit distribution matches cumulative15 subtraction",
        ),
        "next_open_orbit_match": gate(
            bool(open_rows)
            and open_rows[0]["candidate_count"] == 1
            and open_rows[0]["block_count"] == 2,
            "cumulative15 open Serre orbit ranking",
            "the final active Serre orbit is a single-candidate demand",
        ),
        "closed_items_absent": gate(
            ...
            "cumulative15 open frontier",
            "closed candidates and Serre orbits are absent",
        ),
    }
    return {
        "scope": "cumulative15 closure-adjusted selected radius-4 frontier",
        "status": "cumulative15_closure_adjusted_probe_queue_emitted",
        "closures_applied": {
        ...
        "summary": {
            ...
            "cumulative15_adjusted_missing_blocks": sum(row["block_count"] for row in open_rows),
            "raw_unique_missing_serre_orbits": demand["summary"][
                "unique_missing_serre_orbits"
            ],
            "cumulative15_adjusted_unique_missing_serre_orbits": len(open_rows),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Cumulative15 Closure-Adjusted Radius-4 Frontier",
        "",
    ...
    lines.extend(["", "## Final Open Serre Orbit", ""])
    ...
    lines.extend(
        [
            ...
            "",
            (
                "After applying the verified sixteenth branch closure, the selected "
                "radius-4 frontier is reduced to one character-incomplete "
                "single-orbit candidate."
            ),
            "",
        ]
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative15_adjusted_frontier.md"
        ),
    )
