#!/usr/bin/env python3
"""Cumulative selected radius-4 frontier after eight verified branch closures."""
...
CLOSED_CANDIDATES = {
    ...
    "batch2:window3_radius4_adjacency_filtered_2_known_line_resolved",
}
CLOSED_ORBITS = {
    (0, 0, -2, 1, -1, 1, -1),
    (-2, 2, -1, -1, 1, 0, 1),
...
def build_report() -> dict[str, Any]:
    ...
    verification_paths = [
        ...
        REPORTS / "phenomenology_guided_q1_radius4_eighth_orbit_branch_closure_verification.json",
    ]
    ...
    gates = {
        "all_closure_verifications_pass": gate(
            ...
            "cumulative7 frontier subtracts only verified no-viable branch closures",
        ),
        "cumulative_counts_match": gate(
            len(open_candidates) == 9
            and sum(row["block_count"] for row in open_rows) == 32
            and len(open_rows) == 16,
            "cumulative7 open candidate/orbit rows",
            "eighteen branch-closed candidates and nine Serre orbits are removed",
        ),
        "open_distribution_match": gate(
            dict(sorted(open_distribution.items())) == {1: 6, 2: 2, 6: 1},
            "cumulative7 open candidates",
            "open candidate missing-orbit distribution matches cumulative7 subtraction",
        ),
        "next_open_orbit_match": gate(
            open_rows[0]["candidate_count"] == 1 and open_rows[0]["block_count"] == 2,
            "cumulative7 open Serre orbit ranking",
        ...
        "closed_items_absent": gate(
            ...
            "cumulative7 open frontier",
            "closed candidates and Serre orbits are absent",
        ),
    }
    return {
        "scope": "cumulative7 closure-adjusted selected radius-4 frontier",
        "status": "cumulative7_closure_adjusted_probe_queue_emitted",
        "closures_applied": {
        ...
        "summary": {
            ...
            "cumulative7_adjusted_missing_blocks": sum(row["block_count"] for row in open_rows),
            "raw_unique_missing_serre_orbits": demand["summary"][
                "unique_missing_serre_orbits"
            ],
            "cumulative7_adjusted_unique_missing_serre_orbits": len(open_rows),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Cumulative7 Closure-Adjusted Radius-4 Frontier",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            (
                "After eight verified branch-closure waves, all remaining active "
                "Serre-orbit demands remain single-candidate rows. The selected "
                "radius-4 frontier is reduced to nine character-incomplete candidates."
            ),
            "",
        ]
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative7_adjusted_frontier.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative7_adjusted_frontier.md"
        ),
    )
