#!/usr/bin/env python3
"""Exact monoid audit for mass-bound q=1 branches in batch-4 branch analysis."""
...
SOURCE = REPORTS / "phenomenology_guided_q1_radius4_batch4_branch_analysis.json"
...
def build_report() -> dict[str, Any]:
    ...
    targets = [
        record
        for record in source["desired_q1_branch_candidate_records"]
    ...
    gates = {
        "source_branch_report_imported": gate(
            source["all_gates_pass"]
            and source["summary"]["statuses"].get("no_certified_triplet_mass_operator_found")
            == 9,
            str(SOURCE),
            "audit starts from the verified batch-4 branch report with nine mass-bound q=1 branches",
        ),
        "all_mass_bound_branches_audited": gate(
            len(audited) == 9,
            str(SOURCE),
            "all batch-4 branch mass-bound candidates were audited",
        ),
        "all_mass_entries_obstructed": gate(
            all(candidate["all_mass_entries_monoid_obstructed"] for candidate in audited),
            "exact trace-shifted nonnegative singlet monoid solves",
            "every mass-bound branch is upgraded to a certified singlet-monoid obstruction",
        ),
    }
    return {
        "scope": "exact monoid audit for mass-bound q=1 branches from batch-4 branch analysis",
        "status": "batch4_branch_mass_bound_rows_upgraded_to_monoid_obstructions",
        "audited_candidates": audited,
        "summary": {
            "mass_bound_branches": len(targets),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Batch-4 Branch Mass Monoid Audit",
        "",
...
def main() -> int:
    ...
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_batch4_branch_mass_monoid_audit.md"
        ),
    )
