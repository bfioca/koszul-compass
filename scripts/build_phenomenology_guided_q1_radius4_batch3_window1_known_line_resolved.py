#!/usr/bin/env python3
"""Resolve batch-3 radius-4 unresolved records with known line probes."""
...
WINDOW_REPORTS = {
    "window1": REPORTS / "phenomenology_guided_q1_radius4_adjacency_scout_batch3_window1.json",
    "window3": REPORTS / "phenomenology_guided_q1_radius4_adjacency_scout_batch3_window3.json",
    "window4": REPORTS / "phenomenology_guided_q1_radius4_adjacency_scout_batch3_window4.json",
    "window5": REPORTS / "phenomenology_guided_q1_radius4_adjacency_scout_batch3_window5.json",
    "window6": REPORTS / "phenomenology_guided_q1_radius4_adjacency_scout_batch3_window6.json",
}


def unresolved_candidates(window: str, report: dict[str, Any]) -> list[dict[str, Any]]:
    items = []
...
def build_report(windows: list[str]) -> dict[str, Any]:
    ...
    sources_by_window = {window: load_json(WINDOW_REPORTS[window]) for window in windows}
    actuals, sources, conflicts, imported_reports = load_verified_actuals()
    items = []
    for window, source in sources_by_window.items():
        items.extend(unresolved_candidates(window, source))
        ...
        if not certified["character_certified"]:
            filtered["classification"] = {
                ...
                "reason": (
                    ...
                    "missing batch-3 window-1 character block"
                ),
            }
        else:
    ...
    gates = {
        "source_window_verified": gate(
            all(source["all_gates_pass"] for source in sources_by_window.values())
            and all(source["summary"]["viable_count"] == 0 for source in sources_by_window.values())
            and sum(
                source["summary"]["statuses"].get("missing_character_or_charge_level_data", 0)
                for source in sources_by_window.values()
            )
            == len(items),
            ", ".join(str(WINDOW_REPORTS[window]) for window in windows),
            "batch-3 known-line pass starts from verified scout windows",
        ),
        "verified_probe_actuals_imported": gate(
            len(imported_reports) == 7 and len(actuals) >= 70,
            ", ".join(imported_reports),
            "known-line actuals are imported from the verified probe library; global conflicts are recorded and only compatible line actuals are applied",
        ),
        "all_unresolved_records_attempted": gate(
            len(items) == len(filtered_records)
            and len(items)
            == sum(
                source["summary"]["statuses"].get("missing_character_or_charge_level_data", 0)
                for source in sources_by_window.values()
            ...
            ", ".join(str(WINDOW_REPORTS[window]) for window in windows),
            "all selected batch-3 unresolved records were attempted",
        ),
        "per_candidate_sections_emitted": gate(
            ...
            "batch-3 window-1 known-line filtered records",
    ...
    return {
        "scope": "known-line resolution pass over selected batch-3 radius-4 unresolved candidates",
        "batch": "batch3",
        "windows": windows,
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Radius-4 Batch-3 Known-Line Resolution",
        "",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=str(
            REPORTS
            / "phenomenology_guided_q1_radius4_batch3_known_line_resolved.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS
            / "phenomenology_guided_q1_radius4_batch3_known_line_resolved.md"
        ),
    )
