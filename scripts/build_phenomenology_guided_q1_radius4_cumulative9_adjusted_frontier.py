#!/usr/bin/env python3
"""Cumulative selected radius-4 frontier after tenth branch closure."""
...
from build_phenomenology_guided_q1_radius4_cumulative8_adjusted_frontier import (  # noqa: E402
    CLOSED_CANDIDATES as CUMULATIVE8_CLOSED_CANDIDATES,
    CLOSED_ORBITS as CUMULATIVE8_CLOSED_ORBITS,
)

TENTH_TARGET_CANDIDATE = "batch2:window5_radius4_adjacency_filtered_0_known_line_resolved"
TENTH_ORBIT = (-2, 0, -2, 0, -1, 1, 1)

CLOSED_CANDIDATES = set(CUMULATIVE8_CLOSED_CANDIDATES) | {TENTH_TARGET_CANDIDATE}
CLOSED_ORBITS = set(CUMULATIVE8_CLOSED_ORBITS) | {TENTH_ORBIT}
...
def build_report() -> dict[str, Any]:
    ...
    verification_paths = [
        ...
        REPORTS / "phenomenology_guided_q1_radius4_tenth_orbit_branch_closure_verification.json",
    ]
    ...
    gates = {
        "all_closure_verifications_pass": gate(
            ...
            "cumulative9 frontier extends only verified no-viable branch closures",
        ),
        "cumulative_counts_match": gate(
            len(open_candidates) == 7
            and sum(row["block_count"] for row in open_rows) == 26
            and len(open_rows) == 13,
            "cumulative9 open candidate/orbit rows",
            "twenty branch-closed candidates and twelve Serre orbits are removed",
        ),
        "open_distribution_match": gate(
            dict(sorted(open_distribution.items())) == {1: 5, 2: 1, 6: 1},
            "cumulative9 open candidates",
            "open candidate missing-orbit distribution matches cumulative9 subtraction",
        ),
        "next_open_orbit_match": gate(
            bool(open_rows)
            and open_rows[0]["candidate_count"] == 1
            and open_rows[0]["block_count"] == 2,
            "cumulative9 open Serre orbit ranking",
        ...
        "closed_items_absent": gate(
            ...
            "cumulative9 open frontier",
            "closed candidates and Serre orbits are absent",
        ),
    }
    return {
        "scope": "cumulative9 closure-adjusted selected radius-4 frontier",
        "status": "cumulative9_closure_adjusted_probe_queue_emitted",
        "closures_applied": {
        ...
        "summary": {
            ...
            "cumulative9_adjusted_missing_blocks": sum(row["block_count"] for row in open_rows),
            "raw_unique_missing_serre_orbits": demand["summary"][
                "unique_missing_serre_orbits"
            ],
            "cumulative9_adjusted_unique_missing_serre_orbits": len(open_rows),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Cumulative9 Closure-Adjusted Radius-4 Frontier",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            (
                "After applying the verified tenth branch closure, all remaining active "
                "Serre-orbit demands remain single-candidate rows. The selected "
                "radius-4 frontier is reduced to seven character-incomplete candidates."
            ),
            "",
        ]
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative9_adjusted_frontier.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative9_adjusted_frontier.md"
        ),
    )
