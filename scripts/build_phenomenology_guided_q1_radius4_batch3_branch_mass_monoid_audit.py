#!/usr/bin/env python3
"""Exact monoid audit for mass-bound q=1 branches in batch-3 branch analysis."""
...
SOURCE = REPORTS / "phenomenology_guided_q1_radius4_batch3_branch_analysis.json"
...
def build_report() -> dict[str, Any]:
    ...
    gates = {
        "source_branch_report_imported": gate(
            ...
            == 2,
            str(SOURCE),
            "audit starts from the verified batch-3 branch report with two mass-bound q=1 branches",
        ),
        "all_mass_bound_branches_audited": gate(
            len(audited) == 2,
            str(SOURCE),
            "all batch-3 branch mass-bound candidates were audited",
        ),
    ...
    return {
        "scope": "exact monoid audit for mass-bound q=1 branches from batch-3 branch analysis",
        "status": "batch3_branch_mass_bound_rows_upgraded_to_monoid_obstructions",
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Batch-3 Branch Mass Monoid Audit",
        "",
...
def main() -> int:
    ...
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_batch3_branch_mass_monoid_audit.md"
        ),
    )
