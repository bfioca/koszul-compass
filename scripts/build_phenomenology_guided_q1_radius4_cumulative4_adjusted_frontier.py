#!/usr/bin/env python3
"""Cumulative selected radius-4 frontier after five verified branch closures."""
...
def gate(status: bool, evidence: str, note: str) -> dict[str, object]:
...
def build_report() -> dict[str, object]:
    ...
    gates = {
        "all_closure_verifications_pass": gate(
            ...
            "cumulative4 frontier subtracts only verified no-viable branch closures",
        ),
        "cumulative_counts_match": gate(
            len(open_candidates) == 14
            and sum(row["block_count"] for row in open_rows) == 42
            and len(open_rows) == 19,
            "cumulative4 open candidate/orbit rows",
            "thirteen branch-closed candidates and six Serre orbits are removed",
        ),
        "open_distribution_match": gate(
            dict(sorted(open_distribution.items())) == {1: 11, 2: 2, 6: 1},
            "cumulative4 open candidates",
            "open candidate missing-orbit distribution matches cumulative4 subtraction",
        ),
        "next_open_orbit_match": gate(
            open_rows[0]["serre_orbit_representative"] == [0, 0, -2, -2, 0, 1, 1]
            and open_rows[0]["candidate_count"] == 2
            and open_rows[0]["block_count"] == 4,
            "cumulative4 open Serre orbit ranking",
            "next cumulative4 bottleneck is the remaining two-candidate mixed-degree orbit",
        ),
        "closed_items_absent": gate(
            ...
            "cumulative4 open frontier",
            "closed candidates and Serre orbits are absent",
        ),
    }
    return {
        "scope": "cumulative4 closure-adjusted selected radius-4 frontier",
        "status": "cumulative4_closure_adjusted_probe_queue_emitted",
        "closures_applied": {
        ...
        "summary": {
            ...
            "cumulative4_adjusted_missing_blocks": sum(row["block_count"] for row in open_rows),
            "raw_unique_missing_serre_orbits": demand["summary"][
                "unique_missing_serre_orbits"
            ],
            "cumulative4_adjusted_unique_missing_serre_orbits": len(open_rows),
...
def write_markdown(report: dict[str, object], path: Path) -> None:
    lines = [
        "# Cumulative4 Closure-Adjusted Radius-4 Frontier",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            (
                "After five verified branch-closure waves, the selected radius-4 "
                "frontier has one remaining two-candidate Serre-orbit bottleneck. "
                "The active work then becomes single-candidate orbit closure plus "
                "the two multi-orbit candidates still listed in the open distribution."
            ),
            "",
        ]
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative4_adjusted_frontier.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_cumulative4_adjusted_frontier.md"
        ),
    )
