#!/usr/bin/env python3
"""Verify the local two-point deformation segment through K_even."""
...
DIRECTION = [[-1, 1, 0, 0, 0], [2, -2, 0, 0, 0], [-1, 1, 0, 0, 0]]
    ...
    return {
        "family": {
            ...
            "polynomial_certificates": {
                "index_v": "20 n^2 + 20 n - 12",
                "index_wedge2_v": "20 n^2 + 20 n - 12",
                "anomaly": [
                    "12 - 12 n (n + 1)",
                    "12 + 12 n (n + 1)",
                    "12 - 20 n (n + 1)",
                ],
                "hard_gate_integer_segment": [-1, 0],
            },
            "hard_gate_segment": [
                record["n"] for record in records if record["passes_hard_gates"]
            ],
        },
        "records": records,
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(REPORTS / "k_even_local_family.json"),
    )
