#!/usr/bin/env python3
"""Verify the cumulative3 fifth Serre-orbit branch closure report."""
...
def verify() -> dict[str, Any]:
    path = REPORTS / "phenomenology_guided_q1_radius4_fifth_orbit_branch_closure.json"
    md_path = REPORTS / "phenomenology_guided_q1_radius4_fifth_orbit_branch_closure.md"
    ...
    gates = {
        "builder_gates_pass": gate(
            ...
            str(path),
            "builder-side fifth-orbit branch gates passed",
        ),
        "target_orbit_matches_cumulative3_frontier": gate(
            report["fifth_serre_orbit"]["representative"] == [0, -2, 0, -2, 0, 1, 0]
            and report["fifth_serre_orbit"]["dual"] == [0, 2, 0, 2, 0, -1, 0]
            and report["fifth_serre_orbit"]["target_candidates"]
            == [
            ...
            str(path),
            "branch closure targets the verified cumulative3 next Serre orbit",
        ),
        "all_trace_branches_certified": gate(
            ...
            str(path),
            "all two-candidate dimension-two trace branches are fully certified",
        ),
        "classification_counts_match": gate(
            ...
            str(path),
            "all fifth-orbit branches are nonviable with exact statuses",
        ),
        "no_viable_branch_found": gate(
            report["summary"]["viable_count"] == len(viable) == 0
            and report["status"] == "no_viable_candidate_found_in_fifth_serre_orbit_branches",
            str(path),
            "no trace branch of the fifth orbit is viable",
        ),
        "markdown_exposes_closure": gate(
            ...
            str(md_path),
            "markdown exposes fifth-orbit branch count and obstruction statuses",
        ),
    }
    return {
        "scope": "verification for cumulative3 fifth Serre-orbit branch closure",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=str(
            REPORTS
            / "phenomenology_guided_q1_radius4_fifth_orbit_branch_closure_verification.json"
        ),
    )
