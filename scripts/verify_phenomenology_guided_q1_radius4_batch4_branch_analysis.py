#!/usr/bin/env python3
"""Verify batch-4 bounded branch analysis."""
...
def verify() -> dict[str, Any]:
    path = REPORTS / "phenomenology_guided_q1_radius4_batch4_branch_analysis.json"
    md_path = REPORTS / "phenomenology_guided_q1_radius4_batch4_branch_analysis.md"
    ...
    gates = {
        "builder_gates_pass": gate(
            ...
            str(path),
            "builder-side batch-4 branch analysis gates passed",
        ),
        "all_batch4_incomplete_records_evaluated": gate(
            summary["unresolved_records"] == 31
            and summary["records_evaluated"] == 31
            and summary["records_skipped"] == 0
            and summary["branches_evaluated"] == 31227,
            str(path),
            "all batch-4 known-line-incomplete rows are branch-enumerated",
        ),
        ...
        "markdown_exposes_branch_analysis": gate(
            "Batch-4 Branch Analysis" in md_text
            and "branches_evaluated: `31227`" in md_text
            and "records_skipped: `0`" in md_text,
            str(md_path),
            "markdown exposes batch-4 branch-closure totals",
        ),
    }
    return {
        "scope": "verification for batch-4 bounded branch analysis",
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_batch4_branch_analysis_verification.json"
        ),
    )
