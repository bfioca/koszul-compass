#!/usr/bin/env python3
"""Branch-close the cumulative10 next mixed-degree Serre orbit."""
...
TWELFTH_ORBIT = (-1, 0, 2, 2, 0, -1, 0)
TWELFTH_ORBIT_DUAL = tuple(-value for value in TWELFTH_ORBIT)
TARGET_CANDIDATE = "batch1:window4_radius4_adjacency_filtered_4_known_line_resolved"
    ...
    for sector_key, sector in resolved.items():
        for cert in sector["line_certificates"]:
            if cert["actual_character_computed"] or not any(cert["cohomology"]):
                continue
            if serre_orbit(cert["line_bundle"]) != TWELFTH_ORBIT:
                still_unresolved.append(
                    {
                        "sector": sector_key,
                        "line_bundle": cert["line_bundle"],
                        "cohomology": cert["cohomology"],
                        "reason": "outside_twelfth_serre_orbit_branch",
                    }
                )
                continue
            ...
            cert["actual"] = actual
            cert["method"] = (
                "twelfth_serre_orbit_bounded_branch_"
...
def build_report() -> dict[str, Any]:
    ...
    cumulative10 = load_json(REPORTS / "phenomenology_guided_q1_radius4_cumulative10_adjusted_frontier.json")
    cumulative10_verification = load_json(
        REPORTS / "phenomenology_guided_q1_radius4_cumulative10_adjusted_frontier_verification.json"
    )
    ...
    for h1_trace in [2, 0, -2]:
        for h2_trace in [2, 0, -2]:
            ...
            certified["twelfth_serre_orbit_branch_resolution"] = resolution
            ...
            filtered = candidate_certificate_from_5259_record(
                label=(
                    f"batch1_{target['filtered']['label']}"
                    f"_twelfth_h1_{h1_trace:+d}_h2_{h2_trace:+d}"
                ),
                record=certified,
                conf=conf,
            )
            ...
            filtered["twelfth_serre_orbit_branch_resolution"] = resolution
            if not certified["character_certified"]:
                filtered["classification"] = {
                    "category": "unresolved",
                    "status": "twelfth_serre_orbit_branch_still_character_incomplete",
                    "reason": (
                        "the cumulative10 next Serre orbit branch was filled, but "
    ...
    next_orbit = cumulative10["summary"]["next_probe_orbit"]
    gates = {
        "cumulative10_frontier_verified": gate(
            cumulative10["all_gates_pass"]
            and cumulative10_verification["all_gates_pass"]
            and next_orbit["serre_orbit_representative"] == list(TWELFTH_ORBIT),
            str(REPORTS / "phenomenology_guided_q1_radius4_cumulative10_adjusted_frontier.json"),
            "twelfth-orbit branch closure targets the verified cumulative10 bottleneck",
        ),
        "target_candidate_loaded": gate(
            f"batch1:{target['filtered']['label']}" == TARGET_CANDIDATE,
            str(SOURCE_REPORT),
            "the single candidate touched by the cumulative10 next orbit is loaded",
        ),
        "all_trace_branches_tested": gate(
            len(branch_records) == 9
            and sum(
                record["twelfth_serre_orbit_branch_resolution"]["filled_block_count"]
                for record in branch_records
            )
            == 18
            and all(
                record["twelfth_serre_orbit_branch_resolution"][
            ...
            == 9,
            "twelfth Serre orbit branch records",
        ...
        "classification_counts_match": gate(
            ...
            "twelfth Serre orbit branch classifications",
            "all twelfth-orbit branches are rejected by the q=1/phenomenology filter",
        ),
        "no_viable_branch_found": gate(
            len(viable) == 0,
            "twelfth Serre orbit branch classifications",
            "no branch of the cumulative10 next orbit produces a viable candidate",
        ),
    }
    return {
        "scope": "bounded branch closure for the cumulative10 next radius-4 Serre orbit",
        "status": "no_viable_candidate_found_in_twelfth_serre_orbit_branches"
        if not viable
        else "viable_candidate_found_in_twelfth_serre_orbit_branches",
        "twelfth_serre_orbit": {
            "representative": list(TWELFTH_ORBIT),
            "dual": list(TWELFTH_ORBIT_DUAL),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Radius-4 Twelfth Serre Orbit Branch Closure",
        "",
        f"Status: `{report['status']}`",
        "",
        f"- orbit representative: `{report['twelfth_serre_orbit']['representative']}`",
        f"- dual: `{report['twelfth_serre_orbit']['dual']}`",
        f"- target candidate: `{report['twelfth_serre_orbit']['target_candidate']}`",
        "",
        "## Summary",
        "",
    ]
    ...
    for record in report["branch_records"]:
        res = record["twelfth_serre_orbit_branch_resolution"]
        lines.append(
            "- "
            f"`{record['label']}`: "
    ...
    lines.extend(
        [
            ...
            "",
            (
                "The cumulative10 bottleneck has nine mixed-degree trace branches. "
...
def main() -> int:
    ...
    parser.add_argument(
        "--json-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_twelfth_orbit_branch_closure.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        default=str(
            REPORTS / "phenomenology_guided_q1_radius4_twelfth_orbit_branch_closure.md"
        ),
    )
